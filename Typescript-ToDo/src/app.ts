...
//Valida o número de letras no input
...
class AppState {
    ...
    protected getTodoList() {
        new TodoList(this.todos, 'NotStarted');
        new TodoList(this.todos, 'InProgress');
...
//Responsável pelas funções do ToDo 
...
class TodoItem {
    ...
    private deleteHandler() {
      ...
      const option = confirm('Deseja deletar? ');
      if(option == true)
      {
  ...
  class TodoList {
    constructor(
      private todoItems: ItemStructure[],
      private type: 'NotStarted' | 'InProgress' | 'Finished',
    ) {
      this.display();
  ...
  class TodoInput {
    ...
    private submitHandler(e: Event) {
      ...
      if (ValidatedText) {
        ...
        appState.addItem(id, ValidatedText, STATUS.NotStarted);
        this.clearFormInput();
      } else {
        this.clearFormInput();
      }
